...
from load_estimate_data import JiraDataset
...
def train():
    ...
    train_dataset = JiraDataset(df=train_df, tokenizer=tokenizer, max_length=CONFIG['max_length'])
...
def validate(model: nn.Module, tokenizer, device, criterion_reg, avg_train_loss, epoch  ):
    val_df = pd.read_csv(CONFIG['val_path'])
    val_dataset = JiraDataset(df=val_df, tokenizer=tokenizer, max_length=CONFIG['max_length'])
    ...
    with torch.no_grad():
        for batch in val_loader:
            ...
            abs_error = np.abs(pred_days - target_days)
    ...
    print(f'FTE MSE: {final_mae:.3f} FTE')
...
def evaluate():
    ...
    test_dataset = JiraDataset(
        df=test_df, tokenizer=tokenizer, max_length=CONFIG["max_length"]
    )
    ...
    if not os.path.exists(CONFIG["model_save_path"]):
        print(
            f"Weights file is not found at {CONFIG['model_save_path']}. Train first."
        )
        return
    ...
    with torch.no_grad():
        for batch in test_loader:
            ...
            targets_days = np.expm1(targets)
            preds_days = np.expm1(preds)

            preds_days = np.clip(preds_days, a_min=0.0, a_max=None)

            all_targets.extend(targets_days)
            all_predictions.extend(preds_days)
    ...
    print(
        f"STD: {std_error:.3f} FTE"
    )
    print(f"R² Score: {r2:.4f}")

    print("\nStats for absolute errors:")
    print(f"  Median error:           {np.median(abs_errors):.3f} FTE")
    print(f"  Мax absolute error: {np.max(abs_errors):.3f} FTE")

    print("\n Accuracy:")
    print(
        f"  ≤ 0.25 FTE (2h):   {(np.mean(abs_errors <= 0.25) * 100):.2f}%"
    )
    print(
        f"  ≤ 0.50 FTE (4h:   {(np.mean(abs_errors <= 0.50) * 100):.2f}%"
    )
    print(
        f"  ≤ 1.00 FTE:   {(np.mean(abs_errors <= 1.00) * 100):.2f}%"
    )
    print(
        f"  > 2.00 FTE: {(np.mean(abs_errors > 2.00) * 100):.2f}%"
    )
